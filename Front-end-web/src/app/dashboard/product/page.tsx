...
import { AddProductFieldType, OptionsDatagridView } from "@/app/lib/Types";
...
import PopUpConfirm from "@/app/ui/components/popupConfirm/PopUpConfirm";
import { columnsProduct } from "@/app/lib/constant/columns";
...
const FormCategory: React.FC<{ open: () => void }> = ({ open }) => {
      ...
      >
        ...
        <Form.Item<AddCategoryType>
          ...
          rules={[
            {
              required: true,
              message: "Please input Description Catergory!",
            },
          ]}
        >
          <Input />
...
export default function ProductPage() {
  const { ProductState, setFilter, Categories, reload } = useProduct();
      ...
      items.map((item) => ({
        ...
        children:
          item.key === "1" ? (
            ...
            <FormCategory open={onchange} />
          ),
      })),
    [Categories]
  );

  const confirmDelete = useCallback((e: any) => {
    axiosInstance()
      .delete(`/product`, {
        data: { idProduct: e },
      })
